...
def get_all_ranking_history(
    range: str = Query("30d", description="时间范围: 24h, 30d, 365d"),
    db: Session = Depends(get_db)
):
    """获取所有游戏的历史排名数据"""
    beijing_now = get_beijing_time()
    ...
    if range == "24h":
        ...
        start_time = beijing_now - timedelta(days=365)
    else:
        start_time = beijing_now - timedelta(days=30)
...
@router.post("/apps")
def add_app(
    itunes_id: str = Query(..., description="iTunes应用ID"),
    name_cn: str = Query(..., description="中文名称"),
    name_en: str = Query("", description="英文名称"),
    ...
    existing = db.query(AppInfo).filter(AppInfo.itunes_id == itunes_id).first()
    if existing:
    ...
    add_log("info", f"管理员 {current_user.username} 添加了新游戏: {name_cn} (ID: {itunes_id})")
