...
async function fetchSpreadsheets() {
  ...
  try {
    ...
    ElMessage.error({ message: '加载失败', duration: 2000 })
  } finally {
    loading.value = false
  }
}
...
function openUploadDialog() {
  if (!userStore.isAuthenticated) {
    ElMessage.warning({ message: '请先登录后上传', duration: 2000 })
    return
  }
...
async function downloadSheet(sheet: any) {
  try {
    ...
    ElMessage.error({ message: '下载失败', duration: 2000 })
  }
}
...
async function toggleStar(sheet: any) {
  ...
  try {
    ...
    ElMessage.error({ message: '操作失败', duration: 2000 })
  }
}

async function toggleFeature(sheet: any) {
  if (!isAdmin.value) {
    ElMessage.warning({ message: '需要管理员权限', duration: 2000 })
    return
  }
...
function openEditDialog(sheet: any) {
  if (!canEdit(sheet)) {
    ElMessage.warning({ message: '无权限编辑', duration: 2000 })
    return
  }
...
function beforeUpload(file: File) {
  ...
  if (file.size > maxSize) {
    ElMessage.error({ message: '文件大小不能超过 5MB', duration: 2000 })
    return false
  }
  return true
}
...
async function submitEdit() {
  ...
  await editFormRef.value.validate(async (valid) => {
    ...
    if (!currentEditingSheet.value) {
      ElMessage.error({ message: '编辑信息无效', duration: 2000 })
      return
    }
    ...
    try {
      ...
      ElMessage.success({ message: '编辑成功', duration: 2000 })
      editDialogVisible.value = false
      await fetchSpreadsheets()
    } catch (error: any) {
      console.error('[Community] 编辑失败:', error)
      ElMessage.error({ message: error.response?.data?.detail || '编辑失败', duration: 2000 })
    } finally {
      editing.value = false
    }
  })
